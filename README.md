...
**4. ВМ** - создаем и запускаем контейнер _pg-client_ (Клиент) с возможностью работы с терминалом, с удалением контейнера после выхода, с подключением к созданной на шаге 2 сети, с развёрнутым PostgreSQL 15. Запускаем _psql_ с подключением к хосту _pg-server_ под пользователем _postgres_. Вводим пароль, указанный на шаге 3:
```
