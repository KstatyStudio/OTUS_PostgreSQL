...
В процессе нагрузочного теста было сгененрировано 398725296 байт журнальных записей. В течение 10 минут должны были выполниться 20 контрольных точек (600 сек. тест / 30 сек. checkpoint_timeout). На каждую контрольную точку приходится примерно 19936264 байт журнальных записей (~ 19 Мб).
