...
Предварительно потребуется создать новую базу данных _otusdb_copy_ и схему _otusch_, т.к. в дамп выгружались две таблицы без опции создания базы данных, при этом используется своя схема _otusch_, отличная от схемы по умолчанию - _public_, и по условиям домашнего задания требуется восстановить только вторую таблицу.
